...
// Generates weekly team performance report (PDF or CSV)
...
import { generateWeeklyTeamReport } from '@/lib/reports/aggregation'
import { weeklyTeamToCSV } from '@/lib/reports/csv'
import { weeklyTeamToPDF, renderPDF } from '@/lib/reports/pdf'
...
export async function POST(request: NextRequest) {
  ...
  if (format === 'csv') {
    const csv = weeklyTeamToCSV(report)
    return new NextResponse(csv, {
      headers: {
        'Content-Type': 'text/csv',
        'Content-Disposition': `attachment; filename="weekly-team-${dateFrom}.csv"`,
      },
    })
  }

  const docDef = weeklyTeamToPDF(report)
  const buffer = await renderPDF(docDef)
  return new NextResponse(buffer as unknown as BodyInit, {
    headers: {
      'Content-Type': 'application/pdf',
      'Content-Disposition': `attachment; filename="weekly-team-${dateFrom}.pdf"`,
    },
  })
}
