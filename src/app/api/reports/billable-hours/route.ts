...
// Generates billable hours summary report (PDF or CSV)
...
import { generateBillableReport } from '@/lib/reports/aggregation'
import { billableToCSV } from '@/lib/reports/csv'
import { billableToPDF, renderPDF } from '@/lib/reports/pdf'
...
export async function POST(request: NextRequest) {
  ...
  const report = await generateBillableReport(
    user.id, role, profile.team_id, profile.name, dateFrom, dateTo, teamId,
  )

  if (format === 'csv') {
    const csv = billableToCSV(report)
    return new NextResponse(csv, {
      headers: {
        'Content-Type': 'text/csv',
        'Content-Disposition': `attachment; filename="billable-hours-${dateFrom}.csv"`,
      },
    })
  }

  const docDef = billableToPDF(report)
  const buffer = await renderPDF(docDef)
  return new NextResponse(buffer as unknown as BodyInit, {
    headers: {
      'Content-Type': 'application/pdf',
      'Content-Disposition': `attachment; filename="billable-hours-${dateFrom}.pdf"`,
    },
  })
}
